...
### Stage 3 — Order Confirmation Emails (implemented)

- `app/agent/tools/order_confirmation_tool.py` sends the order
  confirmation via `EmailService` (retry already built in for Stage 1
  feedback) to both the admin inbox (`settings.feedback_email_to`,
  defaults to `gsiddhant947@gmail.com`) and the customer's email, right
  after `handle_payment_screenshot` validates payment in
  `app/agent/graph.py`.
- `build_order_summary_email()` is a pure function — like
  `app/agent/reply_templates.py`, the email content is built entirely
  from already-known order state, never generated by an LLM. Contains
  customer details, address, Maps link (if given), itemized order with
  subtotal/discount/delivery fee/total (via `format_cart`), delivery
  slot, payment status, and order ID.
- Email failure is logged and does not block confirming the order to
  the customer in-chat — payment already succeeded by that point, so
  the order is valid regardless of whether the notification email
  lands (no database to mark "retry later" against, per the
